...
class Mg2HfConvert(Convert):
    ...
    def _is_legacy_deepseek2_lite_mcore_schema(self, cur_mg_weight, local_idx):
        layer_prefix = f"decoder.layers.{local_idx}.self_attention."
        return (
            f"{layer_prefix}linear_q_proj.weight" in cur_mg_weight
            and f"{layer_prefix}linear_kv_down_proj.weight" in cur_mg_weight
            and f"{layer_prefix}linear_kv_up_proj.weight" in cur_mg_weight
            and f"{layer_prefix}linear_qkv.weight" not in cur_mg_weight
        )

    def _pop_first_existing(self, cur_mg_weight, candidate_keys, desc):
        for key in candidate_keys:
            if key in cur_mg_weight:
                return cur_mg_weight.pop(key)
        raise KeyError(f"Cannot find {desc}. Candidates tried: {candidate_keys}")

    def _get_legacy_moe_expert_keys(self, local_layer_idx, local_expert_idx):
        fc1_key = f"decoder.layers.{local_layer_idx}.mlp.experts.linear_fc1.weight{local_expert_idx}"
        fc2_key = f"decoder.layers.{local_layer_idx}.mlp.experts.linear_fc2.weight{local_expert_idx}"
        return fc1_key, fc2_key
    ...
    def _collect_layer_candidate_keys(self, cur_mg_weight, local_idx):
        layer_prefix = f"decoder.layers.{local_idx}."
        return sorted(
            key for key in cur_mg_weight
            if key.startswith(layer_prefix)
            and (
                ".self_attention." in key
                or ".mlp." in key
                or "norm" in key
                or "layer_norm" in key
            )
        )

    def _check_layer_schema(self, mg_weight, hf_layer_idx, local_idx, mtp_layer_flag=False):
        cur_tp_ep = (self.tp_rank_list[0], self.ep_rank_list[0])
        cur_mg_weight = mg_weight[cur_tp_ep]
        mg_weight_key = self.load_model.get_weight(local_idx)
        ...
        expected_keys = {
            "input_layernorm": mg_weight_key["layers_input_layernorm"],
            "pre_mlp_layernorm": mg_weight_key["layers_self_attention_pre_mlp_layernorm"],
            "linear_qkv": mg_weight_key["layers_self_attention_linear_qkv"],
            "linear_proj": mg_weight_key["layers_self_attention_linear_proj"],
        }

        # Optional keys for MLA / qk norm style models
        optional_map = {
            "kv_layernorm": "layers_self_attention_kv_layernorm",
            "linear_kv_up_proj": "layers_self_attention_linear_kv_up_proj",
            "linear_q_up_proj": "layers_self_attention_linear_q_up_proj",
            "q_layernorm": "layers_self_attention_q_layernorm",
        }
        for alias, mg_key_name in optional_map.items():
            if mg_key_name in mg_weight_key:
                expected_keys[alias] = mg_weight_key[mg_key_name]

        candidates = self._collect_layer_candidate_keys(cur_mg_weight, local_idx)
        result["candidates"] = candidates

        for alias, expected_key in expected_keys.items():
            if expected_key in cur_mg_weight:
                result["found"][alias] = expected_key
            else:
                result["missing"][alias] = expected_key

        layer_prefix = f"decoder.layers.{local_idx}."

        # Heuristic suggestions
        if "pre_mlp_layernorm" in result["missing"]:
            fallback = f"{layer_prefix}mlp.linear_fc1.layer_norm_weight"
            if fallback in cur_mg_weight:
                result["suggestions"].append(
                    ("pre_mlp_layernorm", fallback, "alias")
                )
            fallback = f"{layer_prefix}post_attn_norm.weight"
            if fallback in cur_mg_weight:
                result["suggestions"].append(
                    ("pre_mlp_layernorm", fallback, "alias")
                )

        if "kv_layernorm" in result["missing"]:
            fallback = f"{layer_prefix}self_attention.linear_kv_up_proj.layer_norm_weight"
            if fallback in cur_mg_weight:
                result["suggestions"].append(
                    ("kv_layernorm", fallback, "alias")
                )

        if "linear_qkv" in result["missing"]:
            q_key = f"{layer_prefix}self_attention.linear_q_proj.weight"
            kv_down_key = f"{layer_prefix}self_attention.linear_kv_down_proj.weight"
            kv_proj_key = f"{layer_prefix}self_attention.linear_kv_proj.weight"
            if q_key in cur_mg_weight and kv_down_key in cur_mg_weight:
                result["suggestions"].append(
                    ("linear_qkv", [q_key, kv_down_key], "compose_required")
                )
            elif q_key in cur_mg_weight and kv_proj_key in cur_mg_weight:
                result["suggestions"].append(
                    ("linear_qkv", [q_key, kv_proj_key], "compose_required")
                )

        return result

    def debug_schema_check(self, mg_weights, pp_rank):
        layer_list = self.pprank_layer_idxs[pp_rank]
        logger.error("========== SCHEMA CHECK START pp_rank=%s ==========", pp_rank)

        issue_count = 0
        for layer in layer_list:
            _, local_idx = self.layeridx_pprank[layer]
            result = self._check_layer_schema(mg_weights, layer, local_idx)
            ...
            issue_count += 1
            logger.error(
                "[layer %s local_idx=%s tp_ep=%s] missing=%s",
                result["hf_layer_idx"],
                result["local_idx"],
                result["tp_ep"],
                result["missing"],
            ...
            logger.error(
                "[layer %s] candidates=%s",
                result["hf_layer_idx"],
                result["candidates"],
            )
            logger.error(
                "[layer %s] suggestions=%s",
                result["hf_layer_idx"],
                result["suggestions"],
            )

        logger.error(
            "========== SCHEMA CHECK END pp_rank=%s, layers_with_issues=%s ==========",
            pp_rank,
            issue_count,
        )
    ...
    def _is_legacy_deepseek2_lite_mcore_schema(self, cur_mg_weight, local_idx):
        layer_prefix = f"decoder.layers.{local_idx}.self_attention."
        has_q_proj = f"{layer_prefix}linear_q_proj.weight" in cur_mg_weight
        has_kv_down = f"{layer_prefix}linear_kv_down_proj.weight" in cur_mg_weight
        has_kv_up = f"{layer_prefix}linear_kv_up_proj.weight" in cur_mg_weight
        has_linear_qkv = f"{layer_prefix}linear_qkv.weight" in cur_mg_weight
        return has_q_proj and has_kv_down and has_kv_up and not has_linear_qkv
    ...
    def set_model_layer_norm(self, hf_weight, mg_weight, hf_layer_idx, local_layer_idx, mtp_layer_flag=False):
        ...
        input_norm = cur_mg_weight.pop(input_norm_key)

        if pre_mlp_norm_key in cur_mg_weight:
            pre_mlp_norm = cur_mg_weight.pop(pre_mlp_norm_key)
        else:
            layer_prefix = f"decoder.layers.{local_layer_idx}."
            pre_mlp_norm = self._pop_first_existing(
                cur_mg_weight,
                [
                    f"{layer_prefix}mlp.linear_fc1.layer_norm_weight",
                    f"{layer_prefix}post_attn_norm.weight",
                    f"{layer_prefix}pre_mlp_layernorm.weight",
                ],
                desc=f"pre_mlp_norm for layer {hf_layer_idx}",
            )
            logger.warning(
                "Using legacy pre-MLP norm fallback for layer %s (local_idx=%s)",
                hf_layer_idx,
                local_layer_idx,
            )
    ...
    def set_model_layer_attn(self, hf_weight, mg_weight, hf_layer_idx, local_layer_idx, mtp_layer_flag=False):
        ...
        if self.load_model.qkv_type == "pack_mla":
            ...
            if not mtp_layer_flag and self._is_legacy_deepseek2_lite_mcore_schema(cur_mg_weight, local_layer_idx):
                layer_prefix = f"decoder.layers.{local_layer_idx}.self_attention."

                q_proj = cur_mg_weight.pop(f"{layer_prefix}linear_q_proj.weight")
                kv_a_proj_with_mqa = cur_mg_weight.pop(f"{layer_prefix}linear_kv_down_proj.weight")
                kv_b_proj = cur_mg_weight.pop(f"{layer_prefix}linear_kv_up_proj.weight")
                kv_a_layernorm = cur_mg_weight.pop(f"{layer_prefix}linear_kv_up_proj.layer_norm_weight")
                o_proj = cur_mg_weight.pop(f"{layer_prefix}linear_proj.weight")

                hf_weight[hf_weight_key["layers_self_attention_linear_q_proj"]] = q_proj.clone()
                hf_weight[hf_weight_key["layers_self_attention_linear_kv_proj"]] = kv_a_proj_with_mqa.clone()
                hf_weight[hf_weight_key["layers_self_attention_linear_kv_up_proj"]] = kv_b_proj.clone()
                hf_weight[hf_weight_key["layers_self_attention_kv_layernorm"]] = kv_a_layernorm.clone()
                hf_weight[hf_weight_key["layers_self_attention_linear_proj"]] = o_proj.clone()

                logger.warning(
                    "Using legacy DeepSeek2-Lite MLA schema fallback for layer %s (local_idx=%s)",
                    hf_layer_idx,
                    local_layer_idx,
                )
                return
    ...
    def set_model_layer_mlp(self, hf_weight, mg_weight, hf_layer_idx, local_layer_idx, mtp_layer_flag=False):
            ...
            if use_grouped_gemm:
                ...
                for (tp_rank, ep_rank) in self.moe_ep_ckpts_list:
                    for local_idx in range(local_expert_nums):
                        ...
                        cur_expert_weight = mg_weight[(tp_rank, ep_rank)]
                        if local_fc1_key not in cur_expert_weight or local_fc2_key not in cur_expert_weight:
                            legacy_fc1_key, legacy_fc2_key = self._get_legacy_moe_expert_keys(local_layer_idx, local_idx)
                            if legacy_fc1_key in cur_expert_weight and legacy_fc2_key in cur_expert_weight:
                                local_fc1_key = legacy_fc1_key
                                local_fc2_key = legacy_fc2_key
                                logger.warning(
                                    "Using legacy MoE expert keys for layer %s expert %s (tp=%s, ep=%s)",
                                    hf_layer_idx,
                                    local_idx,
                                    tp_rank,
                                    ep_rank,
        ...
        cur_tp_ep = (self.tp_rank_list[0], self.ep_rank_list[0])
        cur_mg_weight = mg_weight[cur_tp_ep]
        dense_fc1_key = f"decoder.layers.{local_layer_idx}.mlp.linear_fc1.weight"
        dense_fc2_key = f"decoder.layers.{local_layer_idx}.mlp.linear_fc2.weight"

        force_dense_layer = (
            not mtp_layer_flag
            and dense_fc1_key in cur_mg_weight
            and dense_fc2_key in cur_mg_weight
        )

        if force_dense_layer:
            logger.warning(
                "Force dense MLP path for layer %s (local_idx=%s)",
                hf_layer_idx,
                local_layer_idx,
            )

        use_grouped_gemm = self.moe_grouped_gemm
        if not force_dense_layer and hf_layer_idx >= self.first_k_dense_replace:
            experts_weight1_key, experts_weight2_key = _generate_moe_gemm_layer_key(mtp_layer_flag)
            if experts_weight1_key not in cur_mg_weight or experts_weight2_key not in cur_mg_weight:
                logger.warning(
                    "Grouped GEMM expert weights not found for layer %s (local_idx=%s), "
                    "fallback to non-grouped expert path",
                    hf_layer_idx,
                    local_layer_idx,
                )
                use_grouped_gemm = False

        if hf_layer_idx >= self.first_k_dense_replace and not force_dense_layer:
            # moe
            ...
            if use_grouped_gemm:
                ...
                for ep_rank in self.ep_rank_list:
                    for local_idx in range(local_expert_nums):
                        ...
                        cur_expert_weight = mg_weight[(self.tp_rank_list[0], ep_rank)]
                        if local_fc1_key not in cur_expert_weight or local_fc2_key not in cur_expert_weight:
                            legacy_fc1_key, legacy_fc2_key = self._get_legacy_moe_expert_keys(local_layer_idx, local_idx)
                            if legacy_fc1_key in cur_expert_weight and legacy_fc2_key in cur_expert_weight:
                                local_fc1_key = legacy_fc1_key
                                local_fc2_key = legacy_fc2_key
                                logger.warning(
                                    "Using legacy MoE expert keys for layer %s expert %s (ep=%s)",
                                    hf_layer_idx,
                                    local_idx,
                                    ep_rank,
                                )
                        ...
                        local_gate, local_up = self.linear_fc1_gather_from_tp(
                            mg_weight, local_fc1_key, ep_rank=ep_rank
                        )
                        local_down = self.linear_fc2_gather_from_tp(
                            mg_weight, local_fc2_key, ep_rank=ep_rank
                        )
